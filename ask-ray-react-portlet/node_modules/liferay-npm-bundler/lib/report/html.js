...
function _toConsumableArray(arr) { if (Array.isArray(arr)) { for (var i = 0, arr2 = Array(arr.length); i < arr.length; i++) { arr2[i] = arr[i]; } return arr2; } else { return Array.from(arr); } } /* eslint require-jsdoc: off */
...
function htmlDump(report) {
	...
	var dependencies = htmlSection('Bundled packages', htmlTable('Package', 'Version', 'Copied files', 'Excluded files', 'Exclusions', 'Linked to', Object.keys(_packages).sort().map(function (pkgId) {
		var _packages$pkgId = _packages[pkgId],
		    ...
		    exclusions = _packages$pkgId.exclusions;


		return htmlRow('\n\t\t\t\t\t\t<td>' + name + '</td>\n\t\t\t\t\t\t<td>' + version + '</td>\n\t\t\t\t\t\t<td>' + copiedFiles.length + '</td>\n\t\t\t\t\t\t<td>' + (allFiles.length - copiedFiles.length) + '</td>\n\t\t\t\t\t\t<td>' + exclusions + '</td>\n\t\t\t\t\t\t<td>' + (link === undefined ? '' : link) + '</td>\n\t\t\t\t\t');
	})));

	var packageProcesses = htmlSection('Summary of package transformations', htmlTable('Package', 'Version', 'Pre-babel phase plugins', 'Post-babel phase plugins', 'Babel phase results', Object.keys(_packages).sort().map(function (pkgId) {
	...
	var packageProcessesBundlerDetails = htmlSection.apply(undefined, ['Details of bundler transformations'].concat(_toConsumableArray(Object.keys(_packages).sort().map(function (pkgId) {
		...
		return htmlIf(preKeys.length > 0 || postKeys.length > 0, function () {
			return htmlSubsection('\n\t\t\t\t\t\t\t<a name="' + pkgId + '-pre"></a>\n\t\t\t\t\t\t\t' + pkgId + '\n\t\t\t\t\t\t', htmlIf(preKeys.length > 0, function () {
			...
			}), htmlIf(postKeys.length > 0, function () {
